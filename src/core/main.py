...
def read_in_products_information(G: nx.DiGraph):
    ...
    # Get project root (two levels up from this file: src/core/main.py -> .)
    script_dir = os.path.dirname(os.path.abspath(__file__))
    project_root = os.path.dirname(os.path.dirname(script_dir))
    products_file = os.path.join(project_root, "data", "products.json")
    products_parquet = os.path.join(project_root, "data", "products.parquet")
...
def create_priority_list_from_sales(G: nx.DiGraph, sales_file: str = None) -> IndexedPriorityList:
    ...
    if sales_file is None:
        script_dir = os.path.dirname(os.path.abspath(__file__))
        project_root = os.path.dirname(os.path.dirname(script_dir))
        sales_file = os.path.join(project_root, "data", "Sales_2025.parquet")
